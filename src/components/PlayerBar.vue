  ...
  <div v-if="player.currentTrack" class="h-24 bg-gray-900 border-t border-gray-700 flex items-center justify-center z-50 text-white shadow-2xl relative">
    ...
    <div class="flex items-center gap-6">
      ...
      <div class="flex items-center gap-3 w-[25vw]">
        <span class="text-xs text-gray-400 font-mono w-10 text-right">{{ formatTime(player.currentTime) }}</span>
        ...
        <div class="relative flex-1 h-1.5 group cursor-pointer">
            ...
            <!-- Progress Fill -->
            <div 
                class="absolute left-0 top-0 bottom-0 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full transition-all duration-300"
            ...
            <div 
                ...
                :style="{ left: `${(player.currentTime / (player.duration || 1)) * 100}%` }"
            ></div>
        </div>
